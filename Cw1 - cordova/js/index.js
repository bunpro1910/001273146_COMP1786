...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #maxbudget').text($('#page-create #frm-register #maxbudget').val());
...
function registerTrip(e) {
    ...
    let maxbudget = $('#page-create #frm-register #maxbudget').val();
    let risk = $('#page-create #frm-register #risk').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description,maxbudget , Risk, Date) VALUES (?, ?, ?, ? , ?, julianday('${date}'))`;
        tx.executeSql(query, [name, destination, description,maxbudget, risk], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #maxbudget').text(result.rows[0].maxbudget+ " VNĐ");
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #maxbudget`).val(result.rows[0].maxbudget);
...
function updateTrip(e) {
    ...
    let maxbudget = $('#page-detail #frm-update #maxbudget').val();
    let description = $('#page-detail #frm-update #description').val();
    let risk = $('#page-detail #frm-update #risk').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?,maxbudget =?, Risk = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description,maxbudget, risk, id], transactionSuccess, transactionError);
